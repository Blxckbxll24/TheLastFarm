...
public class UILifeBar : MonoBehaviour
{
    ...
    private void CrearBarraVidaCompleta()
    {
        ...
        // BUSCAR EL CANVAS QUE SE LLAMA EXACTAMENTE "Canvas"
        Canvas canvas = GameObject.Find("Canvas")?.GetComponent<Canvas>();

        // Si no existe (por si acaso), crear uno nuevo como antes
        if (canvas == null)
        {
            if (mostrarDebug)
                Debug.LogWarning("No se encontró un Canvas llamado 'Canvas'. Creando uno nuevo...");
        ...
        {
            if (mostrarDebug)
                Debug.LogError("BARRA DE VIDA CREADA DENTRO DEL CANVAS CORRECTO: Canvas");
        }
        ...
        rectContenedor.anchoredPosition = new Vector2(800f, -20f);
        ...
        if (mostrarDebug)
            Debug.LogError("BARRA DE VIDA CREADA CORRECTAMENTE DENTRO DE 'Canvas'");
    }
